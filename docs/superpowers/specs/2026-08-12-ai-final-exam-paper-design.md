...
`2026-08-15-exam-point-guided-curation-and-comprehensive-diversity-design.md` 是本文档的已确认增量规格。它将资料整理进一步收紧为“考核大纲考试考点 → 教学大纲校验 → 按考点定向检索教学资料 → 最小充分知识卡”，并定义题位考查方式和综合题原型系统。涉及资料提取方向、操作细节准入或综合题结构时，以该增量规格为准。
...
| 2026-08-15 | 2.4 | 固化真实试卷验证后的根因改进：新增 `ExamPoint` 考试考点层，资料改为按考点定向检索、相关性四分类和最小充分知识卡；操作细节仅在考纲明确要求时可独立命题；蓝图新增考查方式分布，综合题由全卷主脑分配多种原型并进行同卷及跨卷结构去重。 |
