...
```json
{
  ...
  "knowledge_cards": [
    {
      "name": "纯净知识点名称",
      "assessable_content": ["一个可独立判分的原子事实"]
    }
  ],
  "source_links": []
}
...
Task 7—8 实现补充约束：蓝图先按题型数量和分值确定题位，再按考纲章节权重分配到 L3/知识卡，禁止按知识点数量轮询出题；生成模型只接收 `QuestionGenerationPayload`，题型结构不合格或出现来源话术的题目进入局部重试，默认最多两次。重试耗尽不直接判人工复核，必须进入统一终局确认；只有终局仍确认缺陷才保留 blocker/`needs_review` 状态。
...
| DeepSeek 网络、临时 HTTP 或协议错误 | 正式网关默认最多 4 次请求，按 1/2/4 秒退避；原型资料整理可配置为 2 次请求以控制成本 |
| 模型返回空内容、非 JSON 或顶层非对象 | 分别记录机器可读原因、提供方请求 ID、响应元数据和脱敏摘要，沿用上述最大请求次数与退避策略 |
| 模型对象结构/Schema 错误 | 使用结构纠错请求并沿用上述最大请求次数；耗尽后拒收结果，不写坏缓存或标题式兜底 |
| 单题质量检查失败 | 最多局部修复 2 次，之后执行终局确认；仍有疑点才 `needs_review` |
| 单个资料文件失败 | `OrganizationRun` 可进入 `partial_failed`，其他文件继续 |
| 总分、题数、权重或版本冲突 | 调用模型前直接阻断 |
| 教师取消 | 停止新任务，丢弃迟到结果 |
| Windows 快照原子替换遇到瞬时文件锁 | 保留旧的完整 JSON；原型最多尝试 5 次，按 50/100/200/400 毫秒退避，并记录最终锁定原因 |
...
| 单题生成失败 | 其他题继续，失败题有界重试；达到上限后进入终局确认，仍确认缺陷才进入 `needs_review` |
...
| 2026-08-15 | 0.7 | 原型实现精简全卷语义审查：仅在确定性审查无冲突时调用，语义冲突只触发相关题位局部修复；当时曾探索同卡复用，后续已收紧为可执行蓝图禁止复用原子卡。 |
| 2026-08-16 | 0.8 | 固化原子知识卡、按权重事实容量、跨题型全卷卡片账本、高分题容量分配、双审语义共识、递进式冲突重规划、终局确认和 Windows 快照原子替换约束。 |
...
一张知识卡只承载一个可独立判分的原子事实，`assessable_content` 的结构虽为数组以兼容模型协议，但发布校验必须要求 `minItems=1` 且 `maxItems=1`；该唯一值就是卡片允许生成的 `coverage_atom`。生成阶段不得从卡内继续拆出新原子，更不得补造卡外原子。主脑在生成前建立全卷覆盖与卡片使用账本；该账本跨选择、判断、填空、简答和综合题共享，任何题型节点都不得重置已使用卡片状态。可执行蓝图禁止同一 `card_id` 出现在多个题位；兼容卡不足时必须在调用规划模型前阻断，要求教师补充有效知识卡或调整题型/题数。高分题按考点尚未满足的分值压力与剩余原子卡容量获得多卡，综合题可以跨考点组合，但每个考点都必须有独立考查原子、答案边界和分值归属。规划器优先阻断重复，而不是生成后整卷返工。

### 20.3 分题型生成

主脑生成任务后由 LangGraph 分发到题型节点。填空题固定预留 `remember`/`understand` 认知层次，偏向理论记忆、定义、术语、条件和基本结论，不生成开放场景、方案设计或多步骤应用。综合题保留基础理解、灵活应用和实际场景的组合能力，并独立生成答案、解析和逐点评分细则。
...
2. 确定性检查确认冲突时直接进入局部修复；没有确定性冲突时再进行宽松语义审查，并采用两个独立审查结果的共识，双方都确认冲突才触发自动修复；
3. 冲突只标记相关题位。先换入全卷未使用且与冲突对象语义更远的原子卡；仍冲突时重新规划该题的考查原子与答案边界，再重新生成，不能只改题干措辞；
4. 重生成达到上限后执行终局确认；先前已确认的冲突只有连续两次独立阴性确认才可清除，任一次仍确认冲突都保留 `needs_review`。不得因重试计数耗尽而无条件升级教师复核，也不得让单次阴性波动把缺陷伪装为合格题。
...
- 一张知识卡只有一个原子 `assessable_content`；可执行蓝图不得复用 `card_id`，兼容卡不足时在模型调用前阻断；
- 跨题型规划不得重置卡片使用账本；高分题按考点分值压力和剩余卡容量分配多卡；
- 填空题必须保留 `remember`/`understand` 理论题位，不得携带 `apply`、`analyze` 或场景设计要求；
- 题干、选项、答案和解析中的答案核心不得泄漏到其他题；
- 语义冲突先换未使用且语义更远的卡，再重规划考查原子和答案边界；只改题干必须继续判冲突；
- 宽松语义审查只有双审共识才触发修复，达到上限后只有终局确认仍有疑点才进入教师复核；
...
试卷不再设置一个全卷统一难度。蓝图中的每个题型独立配置 `difficulty_distribution`：
...
```json
{
  "single_choice": {
    "count": 15,
    "score": 2,
    "difficulty_distribution": {"low": 40, "medium": 40, "high": 20}
  }
}
```

每个题型的低、中、高比例必须合计 100%，使用最大余数法换算为题数。占比换算只影响该题型的难度配额，不改变章节考核权重和总分计算。

### 21.2 排序与生成

每个题型内部固定按“低难度 → 中难度 → 高难度”排序。蓝图输出的 `PlanItem.difficulty` 是题位级属性，生成载荷只读取当前题位难度；不同题型之间互不排序。

未填写难度比例的旧请求兼容为该题型 100% 中等难度，不存在全卷难度回退字段。教师确认蓝图时需要同时查看题型数量、章节分值和每种题型的难度题数。

### 21.3 回归夹具

- 15 道选择题按 40/40/20 分配为 6/6/3，并按低、中、高顺序排列；
- 任一题型的难度比例不合计 100% 时阻断蓝图；
- 旧蓝图未提供比例时全部为中等难度；
- 难度分配不改变章节分值权重；
...
本轮验证确认 Task 1-11 的核心原型模块、节点和回归夹具已经落在代码中：
...
本轮回归已运行：定向 16 项通过，全量 552 项通过（有 3 个依赖弃用警告）。Task 10 真实素材脚本未运行，缺少 `EMBEDDING_BASE_URL`/`EMBEDDING_API_KEY`/`EMBEDDING_MODEL`，不能将其写成通过；前端仍是现有预览，未将 UI 改造写成已完成。

本轮必须保持的回归约束：资料整理模型必须收到 `ExamPoint` 内容；一张知识卡只含一个可命题原子事实，同一证据允许支撑多张原子卡；考点权重先统一为百分比百分点，再按 `max(4, min(20, ceil(weight_percent * 0.7)))` 计算可调事实容量；生成模型只收纯净知识点，来源仅在教师查看区；题干出现实验编号或文件名时根因优先查生成输入，不靠堆课程专属过滤词；考纲权重必须落在蓝图，不能按每张知识卡机械生成一题；不同题型共享全卷 coverage、卡片使用状态和 answer boundary，高分题按考点分值压力与可用卡容量分配，综合题允许受控跨考点；填空题保留 `remember`/`understand` 理论模式；综合题使用原型合同并以结构签名去重；模型空内容、非 JSON、非对象和结构失败有界重试并分别记录安全原因；语义冲突按“换未使用远卡—重规划原子/答案边界—终局确认”修复，宽松审查采用双审共识；Windows `snapshot`/`pipeline.json` 原子替换处理瞬时文件锁；recorder 不得导致重复请求；合法的大题量不能触发递归或状态爆炸。多分值组、共同章节资格集合的蓝图分配已增加规范化剩余容量求解，等资格状态按容量排序折叠，非均匀资格集合仍使用有界迭代搜索。

## 23. 全卷语义多样性与信息独立性

原子知识卡只能保证单题答案边界，不能证明不同卡属于不同主题。知识目录因此增加来源无关的 `concept_cluster`、`answer_proposition`、`relation_edges` 和 `instance_carriers`：

- `concept_cluster` 表示共同考核能力或核心关系，不按文件、章节标题或表面术语拆分；
- `answer_proposition` 表示该卡唯一可评分结论；
- `relation_edges` 保存等价、上下位、组成、对比、汇总和前置关系；
- `instance_carriers` 标记工具、人物、作品、案例或数据集是考纲要求主体还是可替代示例。

蓝图账本由单一 `card_id` 唯一性升级为原子卡、语义概念簇和实例载体三层约束。分配顺序先满足考纲和题型分值，再最大化不同概念簇数量、最小化未被考纲授权的可替代实例曝光，最后才选择具体知识卡。教学资料数量、文件数量和知识卡数量均不能提高主题或实例的考试权重。

生成前使用答案命题关系建立全卷信息依赖图。不同独立题目之间若存在答案等价、单向蕴含，或多道子命题共同推出另一道汇总结论，必须在调用规划和命题模型前阻断。生成后的语义审查只处理模型自然语言表达新增的线索泄露；仅替换具体名称、增加禁词或改写题干不构成有效修复。

考核大纲明确要求掌握的具体对象可以提高实例曝光，但仍不得形成跨题答案依赖。所有集中度约束来自考纲授权、实例角色、可替代性和命题关系，不得使用课程专属过滤词。
