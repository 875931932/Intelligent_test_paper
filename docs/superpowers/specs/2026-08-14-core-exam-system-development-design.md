...
| 2026-08-15 | 0.7 | 原型实现一次精简全卷语义审查：仅在确定性审查无冲突时调用，语义冲突只触发相关题位局部修复；同卡复用新增认知层次唯一校验。 |
...
本轮必须保持的回归约束：资料整理模型必须收到 `ExamPoint` 内容；生成模型只收纯净知识点，来源仅在教师查看区；题干出现实验编号或文件名时根因优先查生成输入，不靠堆课程专属过滤词；考纲权重必须落在蓝图，不能按每张知识卡机械生成一题；不同题型共享全卷 coverage 和 answer boundary，避免答案互泄；填空题固定理论模式；综合题使用原型合同并以结构签名去重；所有模型调用错误只记录安全摘要，recorder 不得导致重复请求；合法的大题量不能触发递归或状态爆炸。多分值组、共同章节资格集合的蓝图分配已增加规范化剩余容量求解，等资格状态按容量排序折叠，非均匀资格集合仍使用有界迭代搜索。
