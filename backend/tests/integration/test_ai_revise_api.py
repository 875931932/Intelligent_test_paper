"""单题 AI 改题端点集成测试（HTTP 层 + 任务落库 + outbox 派发）。

照抄 tests/integration/test_paper_chain_auth.py 的环境模式（SQLite 内存库 +
...
AI_PATH = "/api/v1/courses/{cid}/paper-versions/pv1/items/1/ai-revise"
...
def _seed_paper(factory, cid: str) -> None:
    """直插最小试卷链路（与 unit/test_ai_revise_service 的 fixture 同构）。"""
    with factory() as session:
        with session.begin():
...
def _prepare(env, monkeypatch, *, llm: bool = True):
    """建课程 + 播种试卷链路；返回 (client, cid)。"""
    auth, factory, dispatched = env
    course = auth.post("/api/v1/courses", json={"name": "AI改题课", "slug": "ai-rev"})
    assert course.status_code == 201, course.text
    cid = course.json()["id"]
    _seed_paper(factory, cid)
    monkeypatch.setattr(ai_revise_service, "llm_configured", lambda: llm)
    return auth, cid, dispatched


def test_ai_revise_returns_task_row_and_dispatches(env, monkeypatch):
    auth, cid, dispatched = _prepare(env, monkeypatch)

    resp = auth.post(AI_PATH.format(cid=cid), json={"instruction": "让选项更平行"})
    ...
    assert row.task_type == "ai_revise_item"
    assert row.status == "queued"
    assert row.payload["instruction"] == "让选项更平行"
    assert row.payload["item_index"] == 1
    # outbox 派发必须发生且带 course_id（事务性投递，失败会保持 pending）
    assert dispatched == [cid]

    # 同题同要求的在途请求复用同一任务（双击不重复烧模型）
    again = auth.post(AI_PATH.format(cid=cid), json={"instruction": "让选项更平行"})
    assert again.status_code == 202
    assert again.json()["task_run_id"] == task_id


def test_ai_revise_rejects_blank_instruction(env, monkeypatch):
    auth, cid, _ = _prepare(env, monkeypatch)
    resp = auth.post(AI_PATH.format(cid=cid), json={"instruction": "   "})
    assert resp.status_code == 422


def test_ai_revise_503_when_llm_unconfigured(env, monkeypatch):
    auth, cid, _ = _prepare(env, monkeypatch, llm=False)
    resp = auth.post(AI_PATH.format(cid=cid), json={"instruction": "改"})
    assert resp.status_code == 503


def test_ai_revise_404_for_missing_item(env, monkeypatch):
    auth, cid, _ = _prepare(env, monkeypatch)
    resp = auth.post(
        "/api/v1/courses/{cid}/paper-versions/pv1/items/99/ai-revise".format(cid=cid),
        json={"instruction": "改"},
    )
    assert resp.status_code == 404


def test_ai_revise_409_when_finalized(env, monkeypatch):
    auth, cid, dispatched = _prepare(env, monkeypatch)
    _, factory, _ = env
    with factory() as session:
        session.execute(
