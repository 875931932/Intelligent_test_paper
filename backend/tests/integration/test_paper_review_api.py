"""整卷 AI 质量评审端点集成测试（HTTP 层 + 任务落库 + outbox 派发）。
...
from app.services import paper_review_service
from app.services.auth_service import hash_password

C = "/api/v1/courses/{cid}"
AI_PATH = "/api/v1/courses/{cid}/paper-versions/pv1/ai-review"
...
def _seed_paper(factory, cid: str) -> None:
    """直插最小试卷链路（与 unit/test_paper_review_service 的 fixture 同构）。"""
    with factory() as session:
        with session.begin():
...
def _prepare(env, monkeypatch, *, llm: bool = True):
    ...
    course = auth.post("/api/v1/courses", json={"name": "AI评审课", "slug": "ai-rev"})
    assert course.status_code == 201, course.text
    cid = course.json()["id"]
    _seed_paper(factory, cid)
    monkeypatch.setattr(paper_review_service, "llm_configured", lambda: llm)
    return auth, cid, dispatched


def test_ai_review_returns_task_row_and_dispatches(env, monkeypatch):
    auth, cid, dispatched = _prepare(env, monkeypatch)

    resp = auth.post(AI_PATH.format(cid=cid), json={"instruction": "重点关注难度分布"})
    ...
    assert row.task_type == "review_paper_version"
    assert row.status == "queued"
    assert row.payload["instruction"] == "重点关注难度分布"
    assert row.payload["paper_version_id"] == "pv1"
    # outbox 派发必须发生且带 course_id（事务性投递，失败会保持 pending）
    assert dispatched == [cid]

    # 同卷同关注点的在途请求复用同一任务（双击不重复烧模型）
    again = auth.post(AI_PATH.format(cid=cid), json={"instruction": "重点关注难度分布"})
    assert again.status_code == 202
    assert again.json()["task_run_id"] == task_id


def test_ai_review_allows_empty_instruction(env, monkeypatch):
    # instruction 可空 = 标准评审
    auth, cid, _ = _prepare(env, monkeypatch)
    resp = auth.post(AI_PATH.format(cid=cid), json={"instruction": ""})
    assert resp.status_code == 202, resp.text
    resp = auth.post(AI_PATH.format(cid=cid), json={})
    assert resp.status_code == 202, resp.text


def test_ai_review_503_when_llm_unconfigured(env, monkeypatch):
    auth, cid, dispatched = _prepare(env, monkeypatch, llm=False)
    resp = auth.post(AI_PATH.format(cid=cid), json={"instruction": "评审"})
    assert resp.status_code == 503
    assert dispatched == []  # 503 在建任务之前，不应产生任何派发


def test_ai_review_404_for_missing_paper_version(env, monkeypatch):
    auth, cid, _ = _prepare(env, monkeypatch)
    resp = auth.post(
        "/api/v1/courses/{cid}/paper-versions/pv-x/ai-review".format(cid=cid),
        json={"instruction": "评审"},
    )
    assert resp.status_code == 404


def test_ai_review_404_for_foreign_paper_version(env, monkeypatch):
    # 试卷存在但不属于该课程：同样按 404 处理（课程隔离）
    auth, cid, _ = _prepare(env, monkeypatch)
    other = auth.post("/api/v1/courses", json={"name": "别的课", "slug": "other-c"})
    assert other.status_code == 201, other.text
    other_cid = other.json()["id"]
    resp = auth.post(AI_PATH.format(cid=other_cid), json={"instruction": "评审"})
    assert resp.status_code == 404


def test_ai_review_422_for_empty_paper(env, monkeypatch):
    auth, cid, dispatched = _prepare(env, monkeypatch)
    _auth2, factory, _ = env
    with factory() as session:
        session.execute(paper_versions.insert().values(
            id="pv-empty", course_id=cid, exam_project_id="proj1", version_no=9, status="draft",
        ))
        session.commit()
    resp = auth.post(
        "/api/v1/courses/{cid}/paper-versions/pv-empty/ai-review".format(cid=cid),
        json={"instruction": "评审"},
    )
    assert resp.status_code == 422
    assert "没有题目" in resp.json()["detail"]
    assert dispatched == []  # 空卷拒绝在建任务之前


def test_ai_review_422_for_unknown_body_key(env, monkeypatch):
    auth, cid, _ = _prepare(env, monkeypatch)
    resp = auth.post(AI_PATH.format(cid=cid), json={"review": True})
    assert resp.status_code == 422


def test_ai_review_202_when_finalized(env, monkeypatch):
    # 只读报告无状态禁令：定稿卷照样可发起评审（202）
    auth, cid, dispatched = _prepare(env, monkeypatch)
    _auth2, factory, _ = env
    with factory() as session:
        session.execute(
        ...
        session.commit()
    resp = auth.post(AI_PATH.format(cid=cid), json={"instruction": "定稿前最后把关"})
    assert resp.status_code == 202, resp.text
    assert resp.json()["task_run_id"]
    assert dispatched == [cid]
