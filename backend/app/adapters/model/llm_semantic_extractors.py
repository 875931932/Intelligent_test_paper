        ...
        *,
        ...
        self.client.request_json(
            system_prompt=(
                ...
                "考点知识常以具体参数、配置与方法要点落地，不因写得具体而降级："
                "若 chunk 承载这类事实（如『chunk_size 控制分块长度、chunk_overlap 控制相邻分块重叠』"
                "『删除过长过短或缺元数据的片段并补充章节标题与来源』），"
                "只要该陈述可直接作为该考点的评分答案，一律判 direct；"
                "不要因为陈述带具体数值、形似操作步骤、或出自实验手册就降为 supporting，"
                "也不要因同批多数 chunk 无关而对承载知识的少数条目保守处理。"
                "supporting 只收不构成答案本体的语境：背景与前提、适用条件、常见误用、案例细节、纯操作外壳。"
        ...
        *,
        ...
        self.client.request_json(
            system_prompt=(
                ...
                "禁止把其他片段的内容归属到当前片段。"
                "对每个片段做两件事之一："
                "1) 若该片段承载课程知识，放入 statements，并逐句提取块内全部独立知识点陈述；"
                ...
                "输入片段是聚合大块（常见 1000 字上下、含多个独立知识点）：必须逐条提取块内每一处"
                "独立可评分的知识，密度通常达约每 300~500 字 1 条；禁止择优只挑最显眼的几条"
                "而放弃其余——遗漏的知识在后续环节永远无法找回。"
                "操作步骤叙述中嵌入的机制、格式与参数作用同样是知识（如『通过 apply_chat_template "
                "将 prompt 与 completion 拼接为训练文本格式』『tokenizer 需支持模型的对话模板处理』），"
                "应提取其机制作用，不得因它出现在步骤条目中而略过。"
                "蒸馏出的陈述必须："
                ...
                "content_kind。每个片段最多提取 20 条陈述。返回严格 JSON，不要输出其它键。"
            ),
            payload={
                "material_version_id": material_version_id,
                "chunks": [
