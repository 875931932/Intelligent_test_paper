...
def compile_question_generation_payload(plan_item: PlanItem | CoverageDirective, knowledge_card: dict | None = None) -> QuestionGenerationPayload:
    ...
    if isinstance(plan_item, CoverageDirective):
        if plan_item.question_type == "comprehensive":
            if plan_item.comprehensive_archetype is None:
                raise ValueError("comprehensive directive requires an archetype contract")
            contract = ARCHETYPE_CONTRACTS[plan_item.comprehensive_archetype]
            ...
            output_schema = {
                ...
                "subquestions": {
                    "type": "array",
                    "min_items": plan_item.subquestion_count_range[0],
                    "max_items": plan_item.subquestion_count_range[1],
                    "items": {
                        "type": "object",
                        "required": ["action", "prompt", "answer_boundary", "answer", "rubric"],
                        "properties": {
                            "action": "string",
                            "prompt": "string",
